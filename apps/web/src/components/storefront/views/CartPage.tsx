...
import { updateCartQty, removeCartItem, startCheckout } from '@/components/storefront/actions';
...
export default async function CartPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const items = await readCart(sfId);
  const cartCount = await cartItemCount(sfId);
  ...
  const lines = items.map((i) => {
    ...
    const valid = v != null && v.status === 'published' && v.storefrontId === sfId;
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        ...
        {error && (
          <div className="border border-border rounded p-3 mb-4 text-sm bg-muted text-muted-foreground">
            <strong className="text-foreground">Couldn&apos;t check out:</strong> {error}
          </div>
        )}
        ...
        {lines.length === 0 ? (
          ...
          <>
            <ul className="border-y border-border -mx-4 divide-y divide-border mb-6">
              {lines.map((l) => (
                <li key={l.variantId} className="px-4 py-4">
                  {!l.valid ? (
                    <UnavailableLine sfId={sfId} base={base} variantId={l.variantId} />
                  ) : (
                    <ValidLine
                      sfId={sfId}
                      base={base}
            ...
            <form action={startCheckout.bind(null, sfId, base)}>
              <button
                type="submit"
  ...
  return (
    <div className="flex items-start justify-between gap-3">
      <div className="flex-1 min-w-0">
        <Link href={`${base}/products/${slug}`} className="font-medium truncate hover:underline block">
          {title}
        </Link>
        <div className="text-xs text-muted-foreground font-mono">{sku}</div>
        <form action={updateCartQty.bind(null, sfId, base, variantId)} className="mt-2 flex items-center gap-2">
        ...
        <form action={removeCartItem.bind(null, sfId, base, variantId)} className="mt-1">
          <button type="submit" className="text-xs underline text-muted-foreground hover:text-foreground">
            Remove
          </button>
        </form>
        {oversold && <p className="text-xs text-muted-foreground mt-2">Only {available} in stock.</p>}
      </div>
      <div className="text-right">
        <div className="font-medium tabular-nums">{formatMoney(unitPriceCents * qty, currency)}</div>
...
function UnavailableLine({ sfId, base, variantId }: { sfId: string; base: string; variantId: string }) {
  return (
    <div className="flex items-start justify-between gap-3">
